...
def sync_to_crm(csv_path, sheet_url):
    ...
    if not os.path.exists(CREDENTIALS_FILE):
        ...
        print("Please ensure your Google Service Account credentials file is named 'credentials.json' and is in this directory.")
        sys.exit(1)
    ...
    # ── Write headers if the sheet has no header row ──────────────────────────
    # Fixed: was a complex expression with operator-precedence bug; now a simple,
    # clear check: if row 1 is empty (or missing), write the headers.
    try:
        header_row = sheet.row_values(1)
        if not header_row:
            print("Sheet is empty. Writing headers first...")
            sheet.insert_row(list(headers), 1)
    except Exception as e:
    ...
    for row in data:
        ...
        if b_name.lower() in existing_names:
            skipped_count += 1
            print(f"Skipping (Already exists): {b_name}")
            continue
