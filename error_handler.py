import os
import json
import time
from deploy_agent import deploy_shop_website
from delivery_agent import trigger_delivery_flow

QUEUE_FILE       = "failed_jobs_queue.json"
DEAD_LETTER_FILE = "failed_jobs_dead_letter.json"
MAX_RETRIES      = 3


def log_failed_job(job_type, payload, error_msg):
    """
    Logs a failed job to a local JSON file so it can be retried later.
    Job Types: 'deploy', 'delivery', 'crm_update'
    """
    print(f"\n[ERROR MONITOR] An error occurred in '{job_type}'. Saving to retry queue.")

    # Load existing queue
    queue = []
    if os.path.exists(QUEUE_FILE):
        try:
            with open(QUEUE_FILE, "r") as f:
                queue = json.load(f)
        except Exception:
            queue = []

    # Append new failed job
    job_entry = {
        "id":        str(time.time()),
        "type":      job_type,
        "payload":   payload,
        "error":     str(error_msg),
        "retries":   0,
        "timestamp": time.ctime()
    }

    queue.append(job_entry)

    # Save back to file
    with open(QUEUE_FILE, "w") as f:
        json.dump(queue, f, indent=4)

    print(f"  [*] Saved failed '{job_type}' job to {QUEUE_FILE}.")


def _archive_dead_job(job):
    """Move a permanently-failed job to the dead-letter file and remove from active queue."""
    dead = []
    if os.path.exists(DEAD_LETTER_FILE):
        try:
            with open(DEAD_LETTER_FILE, "r") as f:
                dead = json.load(f)
        except Exception:
            dead = []

    job["dead_at"] = time.ctime()
    dead.append(job)

    with open(DEAD_LETTER_FILE, "w") as f:
        json.dump(dead, f, indent=4)

    print(f"  [Dead Letter] Job {job['id']} archived to {DEAD_LETTER_FILE} after {MAX_RETRIES} failed attempts.")


def process_retry_queue():
    """
    Reads the failed jobs queue and attempts to process them again.
    This script can be run on a scheduled basis (e.g., every 15 minutes).
    Jobs that exceed MAX_RETRIES are moved to a dead-letter file rather than
    kept in the active queue forever.
    """
    if not os.path.exists(QUEUE_FILE):
        print("[RETRY AGENT] Queue is empty. No failed jobs to retry.")
        return

    with open(QUEUE_FILE, "r") as f:
        queue = json.load(f)

    if not queue:
        print("[RETRY AGENT] Queue is empty. No failed jobs to retry.")
        return

    print(f"=========================================")
    print(f" [RETRY AGENT] Processing {len(queue)} failed jobs...")
    print(f"=========================================\n")

    remaining_queue = []

    for job in queue:
        job_type = job.get('type')
        payload  = job.get('payload')
        retries  = job.get('retries', 0)

        # ── Permanently failed: move to dead-letter, do NOT keep in active queue ──
        if retries >= MAX_RETRIES:
            _archive_dead_job(job)
            continue

        print(f"  > Retrying {job_type} job (Attempt {retries + 1}/{MAX_RETRIES})...")
        success = False

        try:
            if job_type == 'deploy':
                url = deploy_shop_website(payload)
                if url:
                    trigger_delivery_flow(payload, url)
                    success = True

            elif job_type == 'delivery':
                lead_data = payload.get('lead_data')
                live_url  = payload.get('live_url')
                trigger_delivery_flow(lead_data, live_url)
                success = True

            elif job_type == 'crm_update':
                email  = payload.get('email')
                ref_id = payload.get('ref_id')
                from webhook import update_crm_payment
                update_crm_payment(email, ref_id)
                success = True

        except Exception as e:
            print(f"  [Error] Retry failed: {e}")
            job['error'] = str(e)

        if success:
            print(f"  [*] SUCCESS! Job {job['id']} completed and removed from queue.")
        else:
            job['retries'] += 1
            remaining_queue.append(job)

    # Save only the jobs that still need to be retried
    with open(QUEUE_FILE, "w") as f:
        json.dump(remaining_queue, f, indent=4)

    print("\n[RETRY AGENT] Queue processing complete.")


if __name__ == "__main__":
    process_retry_queue()
