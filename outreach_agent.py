...
def sanitize_for_speech(name):
    """
    Returns a name safe for use in voice APIs.
    Uses Unicode normalization (NFKD) to transliterate accented chars rather
    than silently dropping them.  'Café Smoke' → 'Cafe Smoke', not 'Caf Smoke'.
    Only falls back to ASCII-drop as a last resort.
    """
    # NFKD decomposes accented characters into base + combining marks;
    # encoding as ASCII then ignores just the combining marks, preserving the base letter.
    normalized = unicodedata.normalize('NFKD', name)
    return normalized.encode('ascii', 'ignore').decode('ascii').strip() or name
...
def dispatch_vapi_call(to_number, business_name, address, reason):
    try:
        ...
        # Use the full unicode-safe name for display; sanitize only for the speech string
        b_name        = clean_business_name(business_name)
        b_name_spoken = sanitize_for_speech(b_name)

        # Reduce address to just city/state for natural speech
        ...
        if "Broken website" in reason:
            first_msg = random.choice([
                f"Hey, is this {b_name_spoken}? I was just trying to pull up your website and it looks like it might be down. Did you guys know about that?",
                f"Hey there, this is Alex. I was trying to find {b_name_spoken} online and the website seems to be having some issues. Is the owner around?",
                f"Hi, is this {b_name_spoken}? Yeah I just tried your website and it's showing an error. Is the person who handles the site available?",
            ])
        elif "No website" in reason:
            first_msg = random.choice([
                f"Hey, is this {b_name_spoken}? I was just searching for you guys online and couldn't find a website. Is the owner in today?",
                f"Hey, this is Alex. I was looking up {b_name_spoken} on Google and noticed there's no website showing up for your shop. Is that something you guys have been thinking about?",
                f"Hi, is this {b_name_spoken}? I was trying to look you guys up online but didn't see a website. Is the owner around by any chance?",
            ])
        else:
            first_msg = random.choice([
                f"Hey, is this {b_name_spoken}? I was looking at your site online and had an idea that could help bring in more walk-ins. Is the owner available?",
                f"Hey, this is Alex. I was checking out {b_name_spoken}'s website and had a thought to help you guys out. Is now an okay time?",
                f"Hi, is this {b_name_spoken}? I was looking at your shop online and was hoping to speak with whoever handles the website side of things.",
            ])

        system_prompt = (
            f"You are Alex, a friendly and professional sales rep for a web design agency that builds websites for smoke shops. "
            f"You are calling {b_name_spoken}. Your goal is to build rapport, pitch the value of a professional website, "
...
def generate_demo_url(business_name, city):
    ...
    Preserves the original unicode name in the URL (percent-encoded as needed).
    """
    b_name = clean_business_name(business_name)
...
def run_outreach(csv_path):
    ...
    for lead in leads:
        # Keep unicode business name intact for display; only sanitize for voice API calls
        b_name = lead.get('business_name', lead.get('Name', 'Unknown')).strip()
